...
"""
Project         : 
File            : clusterByDisease_models.py
Creator         : Ping
...
disease_classification = pd.read_excel("疾病分类.xlsx")

disease_classification_dict = {}

for value in disease_classification.values:
    if value[1] not in disease_classification_dict:
        disease_classification_dict[value[1]] = [value[0]]
    else:
        disease_classification_dict[value[1]].append(value[0])
if not os.path.exists("ClusterByDisease"):
    os.mkdir("ClusterByDisease")
for disease in disease_classification_dict:
    dis_list = disease_classification_dict[disease]
    if not os.path.exists("ClusterByDisease/%s" % disease):
        os.mkdir("ClusterByDisease/%s" % disease)
    positive_spe_dis = [x for x in positive_spe_dis_all if x[1] in dis_list]
    ###
    ...
    for circ in tqdm(range(0, 20)):
        ...
        for train, test in loop:
            ...
            Labels.append(te_y)
        i = 1
        with open("ClusterByDisease/%s/5-fold_gauss_%s_result_%s.csv" %
                  (disease, noise, (circ + 1)), "w") as fw:
