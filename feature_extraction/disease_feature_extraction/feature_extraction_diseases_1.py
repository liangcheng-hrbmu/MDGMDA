...
# os.chdir(r"E:\工作\2020-10-09-5 01_肠道菌-疾病 预测\2021-12-13-特征基于点二列相关\分类model\step1-特征提取\疾病特征提取")

d_m = pd.read_csv('disease_metabolite.txt', sep='\t',
                  dtype=str).sort_values("DOID", ignore_index=True)
...
with open("disease_feature_1.csv.csv", "w") as fw1:
    fw1.write("\t".join(metabolites) + "\n")
    disease_num = len(disease)
    for dis in disease[:9]:
